В командном сотрудничестве очень важно, чтобы каждый четко понимал свои роли и права доступа, чтобы обеспечить бесперебойный прогресс. Сегодня мы изучим создание ролей и управление правами доступа, чтобы сделать командную работу более эффективной и организованной.

Не волнуйтесь — этот процесс прост. Мы проведем вас шаг за шагом, предоставляя руководство на каждом критическом этапе. Если у вас возникнут проблемы, пожалуйста, обращайтесь на наш официальный форум за помощью.

### Анализ требований

Нам нужна роль "Партнер" с определенными правами доступа для участия в управлении задачами, но без возможности свободно изменять задачи других. Такой подход позволяет нам назначать задачи и гибко сотрудничать.
...
>
> Настраивая права доступа ролей, мы можем контролировать поведение пользователей и операции в системе, а также ограничивать отображаемую функциональность, что крайне важно для контроля доступа.
> Использование ролей и прав доступа для привязки пользователей позволяет вам более эффективно управлять вашей системой. Как администратор, вы можете назначать и настраивать права доступа по мере необходимости.

### 6.1 **Создание и назначение ролей**

#### 6.1.1 **Создание роли «Партнер»**

- Нажмите на [**«Пользователи и права доступа»**](https://docs.nocobase.com/handbook/users) в правом верхнем углу интерфейса и выберите [**«Роли и права доступа»**](https://docs.nocobase.com/handbook/acl). Здесь мы настраиваем роли и управляем правами доступа.
...
Вы успешно создали новую роль! Далее нам нужно назначить права доступа для этой роли, чтобы убедиться, что они могут участвовать в управлении задачами.

#### 6.1.2 **Назначение новой роли себе**

Чтобы убедиться, что права доступа роли работают должным образом, мы можем сначала назначить эту роль нашей собственной учетной записи для тестирования. Это просто:
...
Поскольку вы назначили себе роль "Партнер", давайте изучим, как переключать роли.
...
### 6.4 Использование **Управления пользователями и правами доступа** для контроля доступа
...
Теперь вернемся на страницу, чтобы проверить:

Идеально! Когда мы переключаемся на роль Партнер и снова посещаем страницу, только задачи, в которых мы являемся ответственным лицом, можно правильно редактировать.
...
Не беспокойтесь — помните, мы назначили роли Партнер **права доступа на просмотр всех данных** ранее?
...
Если вы переключаетесь между разными пользователями для просмотра формы, вы заметите, что разделы формы автоматически настраиваются, чтобы показывать только операции, разрешенные для каждого пользователя. Все задачи, за которые отвечает пользователь, будут отображать опции редактирования, в то время как задачи вне его ответственности будут показывать только опции просмотра.

Переключение обратно на роль Root восстанавливает полные права доступа. Это мощная система контроля прав доступа NocoBase!

Далее вы можете свободно назначать ответственность за задачи, приглашать участников команды к сотрудничеству и тестировать установленные нами права доступа.
...
Пусть Том войдет в систему, чтобы проверить, может ли он просматривать и редактировать назначенные ему задачи. Согласно правилам прав доступа, Том должен иметь возможность редактировать только задачи, за которые он отвечает, в то время как все остальные задачи доступны только для чтения.
...
Поздравляем! К настоящему моменту вы научились создавать роли, назначать права доступа и настраивать пользовательские права доступа в NocoBase, гарантируя, что члены команды могут редактировать только назначенные им задачи. Через эти шаги вы создали четкую, организованную систему управления правами доступа для командного сотрудничества.

### Задача-испытание

В настоящее время Том может просматривать и редактировать назначенные ему задачи, но вы можете заметить, что он **не может оставлять комментарии** или участвовать в обсуждениях задач. Итак, как мы можем назначить Тому права доступа свободно комментировать и участвовать в обсуждениях? Это будет интересная задача!

**Подсказка к задаче:**

Вы можете пересмотреть настройки прав доступа ролей и настроить роль "Партнер", особенно настройки коллекции комментариев, чтобы увидеть, как разрешить Тому права на комментирование, гарантируя, что его другие ограничения задач остаются в силе.

Попробуйте! Мы раскроем решение в следующем разделе.

В следующей главе мы реализуем функцию «Активность участников команды» и представим еще один мощный модуль: [**Рабочий процесс (Workflow)**](https://docs.nocobase.com/handbook/workflow). С помощью Рабочего процесса вы можете включить поток данных, запускать различные действия и автоматизировать сложные бизнес-процессы в вашей системе. Готовы продолжить исследование? Увидимся в [Главе 7: Рабочий процесс](https://www.nocobase.com/en/blog/task-tutorial-workflow)

---

Исследуйте дальше и раскрывайте свое творчество! Если вы столкнетесь с какими-либо проблемами, не забудьте проверить [Официальную документацию NocoBase](https://docs.nocobase.com/) или присоединиться к [Сообществу NocoBase](https://forum.nocobase.com/) для обсуждений.